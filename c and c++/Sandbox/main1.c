...
*/
    ...
    for (size_t i = 1; i < 6; i++)
    {
        for (size_t j = 1; j < 6; j++)
        {
            ...
            for (size_t k = 1; k < 6; k++)
            {
                if (k == i || k == j || city1.distance[j][k] == 0)
                {
                    continue;
                }
                ...
                for (size_t m = 1; m < 6; m++)
                {
                    if (m == i || m == j || m == k || city1.distance[k][m] == 0)
                    {
                        continue;
                    }
                    ...
                    for (size_t n = 1; n < 6; n++)
                    {
                        if (n == i || n == j || n == k || n == m || city1.distance[m][n] == 0)
                        {
                            continue;
                        }
                        ...
                        ways_to_return[iterrator][4] = m;
                        ways_to_return[iterrator][5] = n;
                        distances_to_return[iterrator] = city1.distance[0][i];
                        distances_to_return[iterrator] += city1.distance[i][j];
                        distances_to_return[iterrator] += city1.distance[j][k];
                        distances_to_return[iterrator] += city1.distance[k][m];
                        distances_to_return[iterrator] += city1.distance[m][n];
                        iterrator++;
                    }
                }
            } 
    ...
    return_index = return_shortest(distances_to_return);
    print_output(city1.cities, ways_to_return, return_index);
    printf("With a distance of %dkm\n\n", distances_to_return[return_index]);
